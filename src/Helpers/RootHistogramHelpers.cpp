...
namespace CAP
{
  ...
  {
  if (verbose) printHistoInfo(caller,name,n_x,min_x,max_x);
  if (n_x<1)
    throw HistogramException(name,"Invalid Histo Specs: n_x<1",caller);
  if (min_x>=max_x)
    throw HistogramException(name,"Invalid Histo Specs: min_x>=max_x",caller);
  return true;
  }
  ...
  bool isValid(const String & name,
               ...
               const String & caller, bool verbose=false)
  {
  if (verbose) printHistoInfo(caller,name,n_x,min_x,max_x,n_y,min_y,max_y);
  if (n_x<1)
    throw HistogramException(name,"Invalid Histo Specs: n_x<1",caller);
  if (n_y<1)
    throw HistogramException(name,"Invalid Histo Specs: n_y<1",caller);
  if (min_x>=max_x)
    throw HistogramException(name,"Invalid Histo Specs: min_x>=max_x",caller);
  if (min_y>=max_y)
    throw HistogramException(name,"Invalid Histo Specs: min_y>=max_y",caller);
  return true;
  }
  ...
  bool isValid(const String & name,
               ...
               const String & caller, bool verbose=false)
  {
  if (verbose) printHistoInfo(caller,name,n_x,min_x,max_x,n_y,min_y,max_y,n_z,min_z,max_z);
  if (n_x<1)
    throw HistogramException(name,"Invalid Histo Specs: n_x<1",caller);
  if (n_y<1)
    throw HistogramException(name,"Invalid Histo Specs: n_y<1",caller);
  if (n_z<1)
    throw HistogramException(name,"Invalid Histo Specs: n_z<1",caller);
  if (min_x>=max_x)
    throw HistogramException(name,"Invalid Histo Specs: min_x>=max_x",caller);
  if (min_y>=max_y)
    throw HistogramException(name,"Invalid Histo Specs: min_y>=max_y",caller);
  if (min_z>=max_z)
    throw HistogramException(name,"Invalid Histo Specs: min_z>=max_z",caller);
  return true;
  }
  ...
  TH1 * createNewHistogram(const String & name,
                           ...
                           bool verbose)
  {
  isValid(name,n_x,min_x,max_x,__FUNCTION__,verbose);
  TH1 * h;
  ...
  TH1 * createNewHistogram(const String & name,
                           ...
                           bool verbose)
  {
  isValid(name,n_x,bins[0],bins[1],__FUNCTION__,verbose);
  TH1 * h;
  switch (storageOption)
    {
  ...
  TH2 * createNewHistogram(const String & name,
                           ...
                           bool verbose )
  {
  isValid(name,n_x,min_x,max_x,n_y,min_y,max_y,__FUNCTION__,verbose);
  TH2 * h;
  switch (storageOption)
    {
  ...
  TH2 * createNewHistogram(const String & name,
                           ...
                           bool verbose)
  {
  isValid(name,n_x,xbins[0],xbins[1],n_y,min_y,max_y,__FUNCTION__,verbose);
  TH2 * h;
  switch (storageOption)
    {
  ...
  TH3 * createNewHistogram(const String & name,
                           ...
                           bool verbose)
  {
  isValid(name,n_x,min_x,max_x,n_y,min_y,max_y,n_z,min_z,max_z,__FUNCTION__,verbose);
  TH3 * h;
  switch (storageOption)
    {
  ...
  void squareDifferenceHistos(TH1 *h1,
                              TH1 *h2,
  ...
  for (unsigned int i=0;i<size;++i) array[i]=v;
  return array;
  }
  ...
  TArrow * createArrow(float x1, float y1, float x2, float y2,
                       ...
                       int style, int color, int width, bool doDraw)
  {
  ...
  void sumHistograms(std::vector<double> a, std::vector<TH1*> histograms,
                     TH1 * sum)
  {
  unsigned int k = 0;
  for (auto h : histograms)
    {
    if (!sameDimensions(__FUNCTION__,h,sum)) IncompatibleHistogramException(__FUNCTION__);
    sum->Add(h,a[k]);
    k++;
    }
  }
  ...
  void differenceHistograms(const TH1 * h1,
                            ...
                            TH1 * difference)
  {
  if (!sameDimensions(__FUNCTION__,h1,h2,difference)) throw IncompatibleHistogramException(__FUNCTION__);
  difference->Add(h1,h2,1.0,-1.0);
  }
  ...
  if (minimum < maximum)
    {
    first = normalized->FindBin(minimum);
    last  = normalized->FindBin(maximum);
    }
  sum = 0.0;
  for (unsigned int k=first; k<=last; k++)
    sum += normalized->GetBinContent(k);
  if (sum<=0.0) throw Exception("<E> partitionHistograms(...) sum<=0.0",__FUNCTION__);
  ...
  void printHistoInfo(const String & source,
                      ...
                      int x_nbins, double x_min, double x_max,
                      int y_nbins, double y_min, double y_max,
                      int z_nbins, double min_z, double max_z)
  {
  printValue("Caller",source);
  printValue("Histogram Name",histoName);
  printValue("x_nbins",x_nbins);
  printValue("x_min",x_min);
  printValue("x_max",x_max);
  if (y_nbins<1) return;
  printValue("y_nbins",y_nbins);
  printValue("y_min",y_min);
  printValue("y_max",y_max);
  if (z_nbins<1) return;
  ...
  printValue("Name",h->GetName());
  printValue("Title",h->GetTitle());
  printValue("Entries",h->GetEntries());
  unsigned int nDim = 0;
  if (h->IsA()==TH1::Class() || h->IsA()==TH1F::Class() || h->IsA()==TH1D::Class() || h->IsA()==TH1I::Class()  || h->IsA()==TProfile::Class() )
    nDim = 1;
  else if (h->IsA()==TH2::Class() || h->IsA()==TH2F::Class() || h->IsA()==TH2D::Class() || h->IsA()==TH2I::Class() || h->IsA()==TProfile2D::Class()  )
    nDim = 2;
  else if (h->IsA()==TH3::Class() || h->IsA()==TH3F::Class() || h->IsA()==TH3D::Class() || h->IsA()==TH3I::Class())
    nDim = 3;
  TAxis * axis;
  ...
  switch (nDim)
    {
      default:
      case 0:
      throw Exception("Histogram dimensionality not recognized",__FUNCTION__);

      case 1:
      {
      axis = h->GetXaxis();
      printValue("X Title",axis->GetTitle());
      printValue("X nbins",axis->GetXbins());
      printValue("X Minimum",axis->GetXmin());
      printValue("X Maximum",axis->GetXmax());
      //        axis = h->GetYAxis();
      //        printValue("Y Title",axis->GetTitle());
      //        printValue("Y Minimum",axis->GetXmin());
      //        printValue("Y Maximum",axis->GetXmax());
      break;
      }
      ...
      printValue("Y Maximum",axis->GetXmax());
      axis = h->GetZaxis();
      //        printValue("Z Title",axis->GetTitle());
      //        printValue("Z nbins",axis->GetXbins());
      //        printValue("Z Minimum",axis->GetXmin());
      //        printValue("Z Maximum",axis->GetXmax());
      break;
      }
