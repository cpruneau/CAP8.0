...
double  chiSquareQuantile(double  p, double  ndf)
{
  ...
  return ch;
    ...
    //! @param x0 The location parameter \f$_x0\f$
    //!
    //!
//! Landau PDF.
...
//!  \param[in] _type        method to compute (from 1 to 9).
///
//! #### NOTE:
...
//!  - Discontinuous:
//!    - _type=1 - inverse of the empirical distribution function
//!    - _type=2 - like _type 1, but with averaging at discontinuities
//!    - _type=3 - SAS definition: nearest even order statistic
...
//!     -_type=4 - linear interpolation of empirical cdf, p(k)=k/n;
//!    - _type=5 - a very popular definition, p(k) = (k-0.5)/n;
//!    - _type=6 - used by Minitab and SPSS, p(k) = k/(n+1);
//!    - _type=7 - used by S-Plus and R, p(k) = (k-1)/(n-1);
//!    - _type=8 - resulting sample quantiles are approximately median unbiased
//!               regardless of the distribution of x. p(k) = (k-1/3)/(n+1/3);
//!    - _type=9 - resulting sample quantiles are approximately unbiased, when
...
void quantiles(int  n, int  nprob, double  *x, double  *quantiles, double  *prob, bool isSorted, int  *index, int  _type)
{
  ...
  // (older implementation had a bug for _type =3)
  ...
  for (int  i=0; i<nprob; i++){
    ...
    if (_type < 4 ){
      ...
      // for continuous quantiles  _type 4 to 9)
      // define alpha and beta
      double a = 0;
      double b = 0;
      if (_type == 4)       { a = 0; b = 1; }
      else if (_type == 5)  { a = 0.5; b = 0.5; }
      else if (_type == 6)  { a = 0.; b = 0.; }
      else if (_type == 7)  { a = 1.; b = 1.; }
      else if (_type == 8)  { a = 1./3.; b = a; }
      else if (_type == 9)  { a = 3./8.; b = a; }
...
double crystalBallPdf(double x, double alpha, double n, double sigma, double mean = 0)
{
  ...
  double fDistributionPdf(double x, double n, double m, double x0)
  {
    ...
    return std::exp((n/2) * std::log(n) + (m/2) * std::log(m) + lgamma((n+m)/2) - lgamma(n/2) - lgamma(m/2)+ (n/2 -1) * std::log(x-x0) - ((n+m)/2) * std::log(m +  n*(x-x0)) );
    ...
    //! Mathworld</A>.
    //!
    double gammaPdf(double x, double alpha, double theta, double x0)
    {
 ...
 double gaussianPdf(double x, double sigma = 1, double x0 = 0)
{
  ...
  using gaussian = gaussianPdf;
    ...
    //!
  double biGaussianPdf(double x, double y, double sigmax, double sigmay, double rho, double x0, double y0)
  {
    ...
    //! @param x0  location parameter, corresponds approximately to the most probable value. For x0 = 0, sigma = 1, the _xmpv = -0.22278
    //!
