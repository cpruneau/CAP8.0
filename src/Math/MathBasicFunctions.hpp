...
namespace CAP
{
  ...
  template <typename T>
  T isFinite(T x) { return std::isfinite(x); }

  template <typename T>
  bool isNaN(T x) { return std::isnan(x);}
...
//  template bool isNaN<long double>(long double x);
  ...
  double power(TX base, TP exponent) { return std::pow(base,exponent);}
//  template long double  power(long double  x, long double  y);
//  template long double  power(long double  x, long y);
//  template long double  power(long x,         long y);
//  template double       power(double x,       double y);
//  template double       power(double x,       int  y);
...
//  }
  ...
  inline T1 copySign( T1 a, T2 b)
  { return (isNegative(b)) ? - absolute(a) : absolute(a); }
  ...
  //!
  inline Double_t copySign(Double_t a, Double_t b)
  { return std::copysign(a,b);  }
  ...
  //!
  inline LongDouble_t copySign(LongDouble_t a, LongDouble_t b)
  { return std::copysign(a,b);  }
