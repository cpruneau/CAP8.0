...
int RunDerived(TString configFile="Pythia/pp_13.7TeV/RA.ini",
               ...
               long seed=12312231)
{
  ...
  workTask->setRequestedConfiguration(&configuration);
  workTask->loadRequestedConfiguration();
  workTask->initialize();
  workTask->execute();
  workTask->finalize();
  //delete workTask;
...
void loadParticleDb(const TString & includeBasePath)
{
  ...
  gSystem->Load(includePath+"ParticleTypeTask.hpp");
...
//void loadE xec(const TString & includeBasePath)
//{
//  TString includePath = includeBasePath + "/E xec/";
//  gSystem->Load(includePath+"RunAnalysis.hpp");
//  gSystem->Load("libpythia8.dylib");
//  gSystem->Load("libE xec.dylib");
//}
...
void loadPHSD(const TString & includeBasePath)
{
  TString includePath = includeBasePath + "/PHSD/";
  gSystem->Load(includePath+"PHSDEventReader.hpp");
  gSystem->Load("libPHSD.dylib");
}
